...
\deq{
	...
	&=\sum_{d=0}^\infty q^d\sum_{\l\prt d} e^{\b f_2(\l)}\left(\frac{\dim \l}{d!}\right)^{2-m}\sum_{\q_1\prt d}\cdots\sum_{\q_{m}\prt d}p_1^{\q_1}\cdots p_{m}^{\q_{m}}\prod_{i=1}^{m}d!|C_i|\c^\l(C_i)\\
...
\deq{\w(p_j,p'_j,q,\b)&=\sum_{d=0}^\infty q^d\sum_{\l\prt d} e^{\b f_2(\l)}s_\l(p_1,\ldots,p_d)s_\l(p'_1,\ldots,p'_d)\\
...
\deq{\fS_\bullet(\l):=\{\l_i-i+1/2\}\subset\Z+\frac12}
...
\deq{\w(p_j,p'_j,q,\b)=\sum_{\l}q^{|\l|} e^{\b f_2(\l)}s_\l(P)s_\l(P')}
